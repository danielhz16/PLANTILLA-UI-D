...
import { AUTH_OPTIONS, TYPES_AUTHORIZATIONS } from '@features/auth/const/auth';
import { Input, f } from '@shared/types/form';
import { Options } from '@shared/types/options';
import { ColumnConfig } from '@shared/types/table';

export interface ModalUsersData {
  idPermission: number;
}

interface UserPermissionVM extends Record<string, unknown> {
  id: number;
  user: string;
  auth: number;
}
    ...
    <div class="ale-relation-modal">
      <div class="ale-relation-header">
        <div class="ale-relation-text">
          <div class="ale-relation-title">Usuarios</div>
          <div class="ale-relation-desc">Gestiona los usuarios asignados a este permiso</div>
        </div>
      ...
      <div class="ale-relation-body">
        @if (isLoading()) {
          ...
          <div class="ale-relation-empty">No hay usuarios asignados a este permiso.</div>
        } @else {
          <app-main-table
            [data]="rows()"
            [columns]="columns"
...
export class ModalUsersComponent implements OnInit {
  readonly rows = signal<UserPermissionVM[]>([]);
  readonly options = signal<Options[]>([]);
  readonly isLoading = signal(true);
  readonly saving = signal(false);

  readonly columns: ColumnConfig<UserPermissionVM>[] = [
    { header: 'Usuario', key: 'user', id: 'user' },
    {
  ...
  private readonly assignedUrl: string;
  ...
  constructor(
    @Inject(MAT_DIALOG_DATA) private readonly dialogData: ModalUsersData,
    private readonly dialogRef: MatDialogRef<ModalUsersComponent>,
    private readonly getQuery: GetQueryService,
    private readonly request: RequestService,
    private readonly modal: ModalService,
  ) {
    this.assignedUrl = `/permissions/users/${this.dialogData.idPermission}`;
    this.availableUrl = `/permissions/available-users/${this.dialogData.idPermission}`;
    this.assignedQuery = this.getQuery.get<Record<string, unknown>[]>(this.assignedUrl);
  ...
  onSubmit(data: Record<string, unknown>): void {
    ...
    const request = this.request.create({
      ...
      method: 'POST',
      keyCache: this.assignedUrl,
    });

    void request
      .execute({
        user,
        auth: Number(data['auth'] ?? TYPES_AUTHORIZATIONS.Read),
        permission: this.dialogData.idPermission,
      })
      .then(async () => {
        this.formRef?.close();
        await this.refresh();
  ...
  private formInputs(): Input[] {
    return [
      f.select('user', this.options()).label('Usuario').required().build(),
      f.select('auth', AUTH_OPTIONS).label('Nivel de autorización').required().build(),
    ];
  }
  ...
  private async refresh(): Promise<void> {
    ...
    try {
      ...
      this.rows.set((this.assignedQuery.data() ?? []).map((row, index) => ({
        id: Number(row['idUserPermission'] ?? index + 1),
